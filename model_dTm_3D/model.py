...
class Encoder(torch.nn.Module):
    ...
    def __init__(self, node_dim, n_head, pair_dim, num_layer):
        ...
        self.finetune_p = torch.nn.Sequential(torch.nn.Linear(node_dim + 1, node_dim), torch.nn.LeakyReLU(), torch.nn.Linear(node_dim, node_dim))
        self.pair_encoder = torch.nn.Linear(7, pair_dim)
        self.blocks = torch.nn.ModuleList([PretrainGeometricAttention(node_dim, n_head, pair_dim) for _ in range(num_layer)])
    ...
    def forward(self, fixed_embedding, dynamic_embedding, pair, atom_mask, mut_pos):
        ...
        embedding = self.fixed_embedding_transform(torch.cat((self.esm2_transform(dynamic_embedding), fixed_embedding[:, :, :-1]), dim=-1))
        embedding = self.finetune_p(torch.cat((embedding, fixed_embedding[:, :, -1:]), dim=-1))
...
class PretrainModel(torch.nn.Module):
    ...
    def forward(self, wt, mut):
        ...
        wt_Tm = self.encoder(torch.cat((wt["fixed_embedding"][:, :, :-2], wt_node_feat, wt["fixed_embedding"][:, :, -2:-1]), dim=-1), wt["dynamic_embedding"], wt["pair"], wt["atom_mask"], wt["mut_pos"])
        mut_Tm = self.encoder(torch.cat((mut["fixed_embedding"][:, :, :-2], mut_node_feat, mut["fixed_embedding"][:, :, -2:-1]), dim=-1), mut["dynamic_embedding"], mut["pair"], mut["atom_mask"], mut["mut_pos"])

        return (mut_Tm - wt_Tm) * self.finetune_rmse_coef
